...
makeQMCinput <- function(plant, inputfile=NA, outputfile=NA, writefile=TRUE){
	...
	for(i in 1:plant$nleaves){
		...
		id <- paste(i, "E(0)")
		write(id, file=inputfile, append=TRUE)
		write("Dbegin", file=inputfile, append=TRUE)
		write("polygon", file=inputfile, append=TRUE)
		...
		write.table(xyz, file=inputfile, append=TRUE, sep=" ", 
			row.names=FALSE, col.names=FALSE)
		write("Dend", file=inputfile, append=TRUE)
	}
	write("Control: 8\n", file=inputfile, append=TRUE)
	
	if(!writefile){
		outr <- readLines(inputfile)
		unlink(inputfile)
